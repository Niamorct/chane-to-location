        ...
        {mob?(
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:4,background:S2,borderRadius:9,padding:3}}>
              {["day","week"].map(m=><button key={m} onClick={()=>setVm(m)} style={{flex:1,background:vm===m?"linear-gradient(135deg,#1a1a2e,#3B82F6)":"transparent",border:"none",color:vm===m?"#fff":"#64748B",padding:"8px 0",borderRadius:7,cursor:"pointer",fontSize:13,fontWeight:600}}>{m==="day"?"📅 Jour":"📆 Semaine"}</button>)}
            </div>
          </div>
        ):(
